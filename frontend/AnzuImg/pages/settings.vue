      ...
      <div v-for="passkey in passkeys" :key="passkey.id"
        ...
        <div class="flex items-start justify-between mb-3">
          ...
          <AnzuButton @click="() => handleDeletePasskey(passkey.CredentialID)" variant="text"
            class="min-w-0! p-2! h-9! w-9! shrink-0 -mr-2 -mt-2 text-(--md-sys-color-error)" :status="deletingPasskeyId === passkey.CredentialID ? 'loading' : 'default'
              ">
    ...
    <div v-else class="grid gap-4 sm:grid-cols-1">
      <div v-for="token in apiTokens" :key="token.id"
        ...
        <div
          ...
          <span>{{ t("settings.apiTokens.created") }}:
  ...
  <div class="max-w-3xl mx-auto mt-10">
    <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <AnzuInput v-model.number="logRetentionDays" type="number" min="1" class="w-28"
          :placeholder="t('settings.apiTokens.cleanupDaysPlaceholder')" />
        <AnzuButton variant="text" :status="cleaningTokenLogs ? 'loading' : 'default'" @click="handleCleanupTokenLogs">
    ...
    <div v-else-if="tokenLogs.length === 0"
    ...
    <div v-else class="space-y-3">
      <div v-for="log in tokenLogs" :key="log.id" class="rounded-xl border border-(--md-sys-color-outline-variant) p-4">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div class="flex items-center gap-2">
            <span
              class="text-sm font-semibold text-(--md-sys-color-on-surface)">{{ getTokenLogActionLabel(log.action) }}</span>
            <span
              class="text-xs px-1.5 py-0.5 rounded bg-(--md-sys-color-surface-variant) text-(--md-sys-color-on-surface-variant)">
        ...
        <div class="mt-2 flex flex-wrap gap-2 text-xs text-(--md-sys-color-on-surface-variant)">
          <span>{{ t("settings.apiTokens.logToken") }}: {{ log.token_name }}</span>
          <span v-if="log.ip_address">{{ t("settings.apiTokens.logIP") }}: {{ log.ip_address }}</span>
          <span v-if="log.image_hash" class="truncate">{{ t("settings.apiTokens.logImage") }}:
            {{ log.image_hash }}</span>
        </div>
        <div class="mt-1 text-xs text-(--md-sys-color-on-surface-variant) opacity-70 truncate">
          {{ log.method }} {{ log.path }}
        </div>
      </div>
    </div>
  ...
  <AnzuDialog v-model:visible="showCreateTokenDialog" :title="t('settings.apiTokens.createNew')" :actions="[
    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-2">
        ...
        <AnzuComboBox v-model="tokenForm.tokenType" :items="tokenTypeOptions"
          :aria-label="t('settings.apiTokens.tokenType')" :placeholder="t('settings.apiTokens.tokenTypePlaceholder')" />
      </div>
...
const handleCreateToken = async () => {
  ...
  if (res) {
    ...
    notify({
      message: t("settings.apiTokens.createFailed"),
...
const handleCleanupTokenLogs = async () => {
  ...
  if (res) {
    ...
    notify({
      message: t("settings.apiTokens.cleanupFailed"),
      type: NotificationType.ERROR,
    });
  }
};
