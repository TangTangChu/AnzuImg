<template>
    <div class="group relative w-full overflow-hidden">
        <div class="flex h-full w-full transition-transform duration-500 ease-[cubic-bezier(0.2,0.0,0,1.0)]"
        ...
        <template v-if="itemCount > 1">
            ...
            <div class="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-2">
                <button v-for="(_, index) in itemCount" :key="index" @click="goToSlide(index)"
                    class="h-1.5 rounded-full transition-all duration-300" :class="[
                        currentIndex === index
