<template>
  <header ref="headerRef" class="fixed top-0 left-0 z-50 w-full transition-all duration-500 ease-out"
    :style="headerStyles">
    <div class="relative z-10 mx-3 my-4 text-(--md-sys-color-on-surface) sm:mx-6">
      <nav class="relative flex flex-nowrap items-center justify-between" role="navigation" aria-label="主导航">
        <div class="flex min-w-0 shrink-0 items-center">
          <NuxtLink to="/"
            ...
            <img src="/favicon.svg" class="mr-2 h-9 shrink-0" alt="logo" />
            <span class="block max-w-56 min-w-0 truncate lg:max-w-[20rem]">
              {{ t("meta.fullName") }}
            </span>
          </NuxtLink>
        </div>

        <div ref="centerColRef" class="relative mx-2 hidden h-10 min-w-0 flex-1 items-center justify-center md:flex">
          <div ref="measureRowRef"
            ...
            <template v-for="(link, index) in primaryLinks" :key="`measure-${link.path}`">
              <div :ref="(el) =>
                setMeasureItemRef(
                ...
                <div v-if="'children' in link" class="flex items-center">
                  <div class="nav-link-block" :class="[
                    ...
                    <span class="relative z-10 flex items-center font-bold">
                      <component :is="link.icon" v-if="link.icon" class="mr-1.5 h-5 w-5" />
                      {{ t(link.label) }}
                    </span>
                  </div>
                  <button type="button"
                    class="ml-0.5 rounded-full p-0.5 transition-colors hover:bg-(--md-sys-color-surface-container-high)">
                    <svg class="h-3.5 w-3.5 text-(--md-sys-color-on-surface-variant) transition-transform" fill="none"
            ...
            <div ref="moreMeasureRef" class="shrink-0">
              <div class="nav-link-block text-(--md-sys-color-on-surface-variant)">
                <span class="relative z-10 flex items-center font-bold">
                  <EllipsisHorizontalIcon class="mr-1.5 h-5 w-5" />
          ...
          <div
            ...
            <template v-for="link in visibleLinks" :key="link.path">
              <AnzuDropdown v-if="'children' in link" :model-value="!!dropdownStates[link.path]"
                @update:modelValue="(v) => (dropdownStates[link.path] = !!v)" width-class="w-48" panel-class="p-1.5"
                open-on-hover :hover-close-delay="180" @mouseenter="moreOpen = false">
                <template #trigger>
                  <div class="flex items-center">
                    ...
                    " class="ml-0.5 rounded-full p-0.5 transition-colors hover:bg-(--md-sys-color-surface-container-high)"
                      :class="{
                        'rotate-180': !!dropdownStates[link.path],
                      }" type="button" aria-label="子菜单">
                      <svg class="h-3.5 w-3.5 text-(--md-sys-color-on-surface-variant) transition-transform" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                      </svg>
                ...
                <template #menu>
                  <NuxtLink v-for="child in (
                    ...
                    class="block rounded-lg px-3 py-2 text-sm transition-colors duration-200" :class="[
            ...
            <AnzuDropdown v-if="overflowLinks.length" v-model="moreOpen" width-class="w-56" panel-class="p-1.5"
              @mouseenter="closeAllDropdowns()">
              <template #trigger>
                <button type="button" class="nav-link-block" :class="moreOpen
                  ? 'active-link'
                  : 'text-(--md-sys-color-on-surface-variant)'
                ...
                <template v-for="link in overflowLinks" :key="`more-${link.path}`">
                  <div v-if="'children' in link" class="p-1">
                    <NuxtLink :to="link.defaultPath || link.path"
                      class="block rounded-lg px-3 py-2 text-sm font-medium transition-colors duration-200" :class="[
                        isActive(link)
                          ? 'bg-(--md-sys-color-secondary-container) text-(--md-sys-color-on-secondary-container)'
                          : 'text-(--md-sys-color-on-surface) hover:bg-(--md-sys-color-surface-container-high)',
                      ]" @click="moreOpen = false">
                      <span class="flex items-center">
                        <component :is="link.icon" v-if="link.icon" class="mr-2 h-5 w-5" />
                        {{ t(link.label) }}
                      </span>
                    </NuxtLink>
          ...
          <div v-if="navTitleBox.title"
            ...
            <div class="text-xs text-(--md-sys-color-on-surface-variant) opacity-70 transition-all duration-500">
        ...
        <div class="ml-2 flex min-w-0 items-center gap-1 sm:ml-4 sm:gap-2">
          ...
          <div v-if="token" class="shrink-0">
            <AnzuDropdown v-model="userMenuOpen" width-class="w-48" panel-class="p-1.5" open-on-hover
              ...
              <template #trigger>
                <button
                  class="flex items-center justify-center rounded-full p-2 text-(--md-sys-color-on-surface) hover:bg-(--md-sys-color-surface-container-high)">
                  <UserIcon class="h-5 w-5" />
                </button>
              </template>
              <template #menu>
                <button
                  class="flex w-full items-center rounded-lg px-3 py-2 text-sm text-(--md-sys-color-on-surface) hover:bg-(--md-sys-color-surface-container-high)"
                  @click="
                    async () => {
                ...
                <button
                  class="flex w-full items-center rounded-lg px-3 py-2 text-sm text-(--md-sys-color-error) hover:bg-(--md-sys-color-error-container)"
                  @click="logout">
                  <ArrowRightEndOnRectangleIcon class="mr-2 h-4 w-4" />
          ...
          <div v-else class="shrink-0">
            <NuxtLink to="/login"
              class="flex items-center rounded-full px-3 py-1.5 text-sm font-bold text-(--md-sys-color-primary) hover:bg-(--md-sys-color-surface-container-high)">
          ...
          <button
            ...
            aria-label="打开菜单" @click.stop="toggleMenu" type="button">
      ...
      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0"
        ...
        <div v-show="isMenuOpen"
          ...
          <template v-for="link in navLinks" :key="`mobile-${link.path}`">
            <div v-if="'children' in link" class="mb-1">
              <div class="mobile-nav-link flex w-full items-center justify-between rounded-lg p-2 transition-colors"
                :class="isActive(link)
                  ...
                  ">
                <NuxtLink :to="link.defaultPath || link.path" class="flex flex-1 items-center" :class="isActive(link)
                ...
                <button @click.stop="toggleMobileSubmenu(link.path)"
                  class="rounded-full p-1 hover:bg-(--md-sys-color-surface-container-highest)" aria-label="子菜单">
                  <svg class="h-4 w-4 text-(--md-sys-color-on-surface-variant) transition-transform" :class="{
                    'rotate-180': mobileDropdownStates[link.path],
              ...
              <div v-if="mobileDropdownStates[link.path]" class="mt-1 mb-2 space-y-1 pl-4">
                <NuxtLink v-for="child in (
                  link as NavLinkWithChildren
                ).children" :key="child.path" :to="child.path"
                  class="block rounded-lg px-3 py-2 text-sm transition-colors" :class="{
                    'bg-(--md-sys-color-secondary-container) font-medium text-(--md-sys-color-on-secondary-container)':
                      route.path === child.path,
            ...
            <NuxtLink v-else :to="link.path" class="mobile-nav-link mb-1 block rounded-lg p-2 transition-colors" :class="[
              isActive(link)
                ? 'bg-(--md-sys-color-surface-container-high) font-medium'
                : 'hover:bg-(--md-sys-color-surface-container-high)/50',
            ]" @click="closeMenu">
              <span class="flex items-center" :class="isActive(link)
                ? 'font-medium text-(--md-sys-color-primary)'
                : 'text-(--md-sys-color-on-surface)'
                ">
                <component :is="link.icon" v-if="link.icon" class="mr-2 h-5 w-5" />
...
const headerStyles = computed(() => {
  ...
  return {
    backgroundColor: `color-mix(in srgb, var(--md-sys-color-surface) ${alphaValue * 100
      }%, transparent)`,
