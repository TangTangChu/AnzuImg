          ...
          <div class="max-w-7xl mx-auto">
            ...
            >
              <div class="min-w-0 flex-1 w-full">
                <h3
                  class="truncate text-xl font-semibold text-(--md-sys-color-on-surface)"
                  :title="displayImage.file_name"
                >
                  {{ displayImage.file_name }}
                </h3>
                <div
                  v-if="displayImage.uploaded_by_token_name"
                ...
                <div
                  class="flex flex-wrap items-center gap-4 mt-2 text-sm text-(--md-sys-color-on-surface-variant)"
                >
            ...
            >
              <div class="md:col-span-2">
                <AnzuAlert v-if="detailError" type="error" class="mb-3">{{
                  detailError
                }}</AnzuAlert>
                <div v-if="loadingDetail" class="flex items-center">
                  <AnzuProgressRing status="loading" :size="20" />
                  <span
                    class="ml-2 text-sm text-(--md-sys-color-on-surface-variant)"
                    >{{ t("gallery.loadingDetails") }}</span
                  >
                </div>
                <div
                  v-else-if="displayImage.description"
              ...
              <div class="space-y-3">
                <div v-if="hasRoutes">
                  <span
                    class="text-xs font-bold text-(--md-sys-color-primary) uppercase"
                    >Routes</span
                  >
                  <div class="flex flex-wrap gap-2 mt-1">
                    <a
                      v-for="route in detailedImage!.routes"
                      :key="route"
                      :href="`/i/r/${route}`"
                      target="_blank"
                ...
                <div>
                  ...
                  >
                  <div
                    v-if="displayImage.tags && displayImage.tags.length > 0"
                    class="mt-1"
                  >
                  ...
                  <div
                    v-else
                    class="mt-1 text-sm text-(--md-sys-color-on-surface-variant) italic"
                  >
                    No tags
                  </div>
                </div>
...
const saveEdit = async () => {
  ...
  try {
    ...
    const parsed = parseApiError(error, t("gallery.detailLoadFailed"));
    detailError.value = parsed.displayMessage;
