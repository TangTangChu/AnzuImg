<template>
    <component :is="tag" ref="buttonRef"
        ...
        :class="[
            ...
            // 变体样式
            effectiveVariant === 'filled' ? 'bg-(--md-sys-color-primary) text-(--md-sys-color-on-primary)' : '',
            effectiveVariant === 'outlined' ? 'border border-(--md-sys-color-outline) bg-transparent text-(--md-sys-color-primary)' : '',
            effectiveVariant === 'text' ? 'bg-transparent text-(--md-sys-color-primary) px-3 hover:bg-(--md-sys-color-primary)/8 active:bg-(--md-sys-color-primary)/12' : '',
            effectiveVariant === 'elevated' ? 'bg-(--md-sys-color-surface-container-low) text-(--md-sys-color-primary) shadow-sm' : '',
            effectiveVariant === 'tonal' ? 'bg-(--md-sys-color-secondary-container) text-(--md-sys-color-on-secondary-container)' : '',
            buttonGroupClasses.includes('horizontal') ? 'rounded-none border-r-0' : '',
            buttonGroupClasses.includes('horizontal') ? 'first:rounded-l-lg last:rounded-r-lg last:border-r' : '',
            buttonGroupClasses.includes('vertical') ? 'rounded-none border-b-0' : '',
            buttonGroupClasses.includes('vertical') ? 'first:rounded-t-lg last:rounded-b-lg last:border-b' : '',
            isButtonDisabled ? 'pointer-events-none cursor-not-allowed border-transparent shadow-none' : '',
        ]" :style="[
            isButtonDisabled ? {
        ...
        <AnzuProgressRing v-if="status === 'loading'" :size="24" :stroke-width="2" :status="status"
            :primary-color="loadingColor" :animation-duration="400" class="relative z-10 shrink-0" />
        <svg v-else-if="status === 'success' && showIcon" width="18" height="18" fill="none" :stroke="loadingColor"
            stroke-width="3" viewBox="0 0 24 24" class="relative z-10 shrink-0">
