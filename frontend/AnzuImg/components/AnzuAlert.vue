<template>
    <div
        ...
        <div v-if="titleText" class="mb-2 flex items-start font-bold text-(--md-sys-color-on-surface)">
            <component :is="iconComponent" class="mt-0.5 mr-2 h-5 w-5 shrink-0" :class="iconColorClass" />
            {{ titleText }}
        </div>
        <div>
            <slot></slot>
...
const titleText = computed(() => {
    ...
    const typeMap: Record<string, string> = {
        succ: t("common.labels.success"),
        warn: t("common.labels.warning"),
        info: t("common.labels.info"),
        error: t("common.labels.error"),
        plain: "",
    };
